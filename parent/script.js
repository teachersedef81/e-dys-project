...
function initializePage() {
    ...
    // Filter to grade 5 by default and render section buttons
    filterClasses('5');
    updateSections('5');
    ...
    // Fetch and show dynamic announcements
    loadParentInfo();
}
...
async function loadParentInfo() {
    try {
        ...
        if (res.ok) {
            ...
            const dropdownArea = document.querySelector('#notificationsDropdown .max-h-96');
            if (dropdownArea && data.length > 0) {
                dropdownArea.innerHTML = data.map(info => {
                    let iconClass = 'fas fa-info';
                    let bgClassBase = 'bg-blue-100';
                    let textClassBase = 'text-blue-600';

                    if (info.type === 'ders_programi') {
                        iconClass = 'fas fa-calendar-alt';
                        bgClassBase = 'bg-purple-100';
                        textClassBase = 'text-purple-600';
                    }
                    if (info.type === 'nobet') {
                        iconClass = 'fas fa-shield-alt';
                        bgClassBase = 'bg-yellow-100';
                        textClassBase = 'text-yellow-600';
                    }

                    return `
                                <div class="p-3 hover:bg-gray-50 border-b border-gray-100">
                                    <div class="flex">
                                        <div class="flex-shrink-0 ${bgClassBase} rounded-full p-2 h-8 w-8 flex items-center justify-center">
                                            <i class="${iconClass} ${textClassBase}"></i>
                                        </div>
                                        <div class="ml-3">
                                            <p class="text-sm font-medium text-gray-800">${info.title}</p>
                                            <p class="text-xs text-gray-500">${info.content}</p>
                                            <p class="text-xs text-gray-400 mt-1">${info.date}</p>
                                        </div>
                                    </div>
                                </div>
                            `;
            ...
            if (data.length > 0) {
                const latest = data[0];
                const bannerContainer = document.querySelector('.notification-banner');
                if (bannerContainer) {
                    bannerContainer.innerHTML = `
                                <span class="font-bold mr-4"><i class="fas fa-exclamation-circle mr-2"></i>ÖNEMLİ DUYURU:</span>
                                <span class="text-sm md:text-base">${latest.title} - ${latest.content}</span>
                            `;
                }
            }
        }
...
function setupEventListeners() {
    ...
    // Section filter buttons are added dynamically in updateSections()
}
...
function filterClasses(gradeLevel) {
    ...
    classCards.forEach(card => {
        ...
        const cardGrade = className.charAt(0);

        if (cardGrade === gradeLevel) {
            card.style.display = '';
        } else {
...
function filterClassesBySection(section) {
    ...
    classCards.forEach(card => {
        const className = card.dataset.className;
        const cardSection = className.charAt(1);

        // Only affect cards that are currently visible (matching grade)
...
function updateSections(gradeLevel) {
    ...
    allBtn.classList.add('section-btn', 'py-2', 'rounded-lg', 'font-medium', 'bg-indigo-600', 'text-white', 'transition', 'hover:bg-indigo-700');
    ...
    sections.forEach(section => {
        ...
        button.classList.add('section-btn', 'py-2', 'rounded-lg', 'font-medium', 'bg-gray-200', 'text-gray-700', 'transition', 'hover:bg-gray-300');
        ...
        button.addEventListener('click', function () {
            ...
            this.classList.remove('bg-gray-200', 'text-gray-700');
            this.classList.add('bg-indigo-600', 'text-white');
            // First show all for this grade, then hide non-matching sections
            filterClasses(gradeLevel);
...
// Initialize the page (only if parent dashboard DOM exists)
